...
//#define USB_JOYSTICK_VENDOR_ID	0x0E8F
//my device id
		      ...
		      const struct usb_device_id *id)
{
	...
	js_dev->data = usb_buffer_alloc(dev, 8, GFP_ATOMIC, &js_dev->data_dma);
	if(!js_dev->data)
		goto fail1;
	gb_data = js_dev->data;
	js_dev->in_urb = usb_alloc_urb(0, GFP_KERNEL);//creat urb
	if(!js_dev->in_urb)
		goto fail2;
	usb_fill_int_urb(js_dev->in_urb, dev, pipe, js_dev->data, (maxp > 8 ? 8 : maxp), joystick_irq, js_dev, endpoint->bInterval);//initlize urb withint type
...
fail2:
	printk("fail2.\n");
	usb_buffer_free(dev, 8, js_dev->data, js_dev->data_dma);
fail1:
	printk("fail1.\n");
	kfree(js_dev);
fail0:
